...
(async () => {
	...
	Highcharts.mapChart('container-europe-co2', {
		...
		series: [{
			name: 'Non EU-27',
			data: [ 'GB', 'CH', 'NO', 'BA', 'SM', 'MC', 'AL', 'RS', 'ME', 'KV', 'MK'].map(code => ({ code }))
		}, {
			name: 'EU-27',
			data: sourceData.map(item => ({code: item.code, value: item.valuec, namefr: item.namefr, valuep: item.valuep}))
		}]
	});
})();

var charts = [],
	containers = document.querySelectorAll('#trellis1 td')

charts.push(Highcharts.chart(containers[0], {
...
plotOptions: {
  ...
  series: [
	{ data: sourceData.slice(0, sourceData.length/2 + 1).map(item => item.valuec) }
]

	
}));

charts.push(Highcharts.chart(containers[2], {
...
plotOptions: {
  ...
  series: [{ data: sourceData.slice(sourceData.length/2 + 1).map(item => item.valuec)}]
}));

containers = document.querySelectorAll('#trellis2 td')
...
charts.push(Highcharts.chart(containers[0], {
  ...
  yAxis: {
	...
	colorAxis: {
		...
		stops: [
			[0, 'rgba(120,120, 120, 0.5'],
			[1, 'rgba(120,120, 120, 0.5']
		]
	},
...
plotOptions: {
  ...
  series: [
	{ data: sourceData.slice(0, sourceData.length/2 + 1).map(item => item.valuep) }
]	}));
...
plotOptions: {
  ...
  series: [{ data: sourceData.slice(sourceData.length/2 + 1).map(item => item.valuep)}]
}));